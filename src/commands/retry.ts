...
async function forEachAsync<T>(
  arr: Array<T>,
  fn: (elem: T) => Promise<void>
): Promise<void> {
  await mapAsync(arr, fn);
}

async function mapAsync<TIn, TOut>(
  arr: Array<TIn>,
  fn: (elem: TIn) => Promise<TOut>
): Promise<Array<TOut>> {
  const result = [];
...
async function getFailedTriggeredBuilds(api: Buildkite, build: Build) {
  const failed = build.jobs.filter(
    (j: Job) =>
      j.type === "trigger" && j.state === "failed" && j.triggered_build
  );
  return await mapAsync(failed, async (b: Build) => {
    const [, org, pipeline, buildNumber] =
      b.triggered_build.url.match(API_BUILD_REGEX);
